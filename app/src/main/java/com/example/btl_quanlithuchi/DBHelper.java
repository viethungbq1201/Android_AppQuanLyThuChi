...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Map<String, Float> getTotalExpenseByCategory() {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT title, SUM(price) as total FROM Infomations WHERE type = 'chi' GROUP BY title", null);

        if (cursor.moveToFirst()) {
            do {
                String category = cursor.getString(cursor.getColumnIndexOrThrow("title"));
